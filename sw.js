...
const CACHE_NAMESPACE = 'hellogithub2014-'
const version = 1;

const CUR_CACHE_NAME = CACHE_NAMESPACE + 'precache-then-runtime-v' + version;
const PRECACHE_LIST = [
    "/",
...
self.addEventListener('install', e => {
    e.waitUntil(
        caches.open(CUR_CACHE_NAME).then(cache => {
...
self.addEventListener('activate', event => {
    ...
    caches.keys().then(cacheNames => Promise.all(
        cacheNames
        .filter(cacheName => cacheName !== CUR_CACHE_NAME)
...
var fetchHelper = {
    ...
    fetchThenCache: function(request) {
        ...
        Promise.all([fetchedCopy, caches.open(CUR_CACHE_NAME)])
...
self.addEventListener('sync', function(event) {
    self.registration.showNotification("所有看过的文章均可离线浏览哦~");
});
