...
![image.png](https://lf1-xgcdn-tos.pstatp.com/obj/vcloud/vadmin/image.7ada7bad1f966abc46085bbd4a616052.png?width=643&height=913)
...
![image.png](https://lf1-xgcdn-tos.pstatp.com/obj/vcloud/vadmin/image.07032cb79b0fd21554505274cb0583fa.png?width=717&height=539)

![image.png](https://lf1-xgcdn-tos.pstatp.com/obj/vcloud/vadmin/image.c18771eecb6fc065d132dac8c9850583.png?width=2850&height=624)
...
1. 如果某个`script`被多个自然语言用到，但业务上这些认定这些自然语言的长度不一致，比如法语中的`p`长度算 2，英语中的`p`长度算 `1`. 如果真的有这种变态需求建议把`PM`锤爆并说这个需求做不了。
   ![做不了.png](https://www.jiuwa.net/tuku/20180404/kANEOyoT.jpg?width=300&height=300)
...
![image.png](https://lf1-xgcdn-tos.pstatp.com/obj/vcloud/vadmin/image.5d5f0794302b420d9ac90aef8823f94a.png?width=1044&height=1470)

发现一些需要注意的：

1. 部分全宽`Latin`字母和`Latin Script`的范围是重合的，如`\uFF21`是全宽的`Ａ`，也被放到`Latin Script`里了
   ![image.png](https://lf1-xgcdn-tos.pstatp.com/obj/vcloud/vadmin/image.b52dd73f457397e35f9eca702a0f99fd.png?width=1602&height=146)
2. 标点符合、数字是不属于任何`script`的
3. 全角和半角字符的范围很零碎，被分为了多段，需要细心分拣

   ![image.png](https://lf1-xgcdn-tos.pstatp.com/obj/vcloud/vadmin/image.882cc4d2e94add263278d30571620393.png?width=2070&height=1352)
...
如果需要更精细的匹配优先级，可以给每个选项设定一个`priority`字段，然后将`SCRIPT_LIST`按`priority`排序，这样可以非常灵活的控制。

## 组合字符串

有些`Unicode`字符是由几个其他字符组合而来的，其中一部分子字符叫做[`Combining character`](https://en.wikipedia.org/wiki/Combining_character) ：

![Cyrillic Combining character](https://upload.wikimedia.org/wikipedia/commons/thumb/4/4e/U_niesk%C5%82adovaje_Unicode.svg/220px-U_niesk%C5%82adovaje_Unicode.svg.png?width=220&height=69)

上图中的“眉毛”就是`Combining character`。再比如泰语的`ณี`是由`ณ`(`\u0E13`)和 `ี` (`\u0E35`)两个字符组成的， `` ี`也是`Combining character`:

![image.png](https://lf1-xgcdn-tos.pstatp.com/obj/vcloud/vadmin/image.b976934cc45922de9fa0f63772eeabd4.png?width=1158&height=1010)
...
![image.png](https://lf1-xgcdn-tos.pstatp.com/obj/vcloud/vadmin/image.9f620051184592c2ca5dc143aab86212.png?width=790&height=871)
...
![image.png](https://lf1-xgcdn-tos.pstatp.com/obj/vcloud/vadmin/image.42b30815922604d70b31841004be6692.png?width=1034&height=249)
...
3. [node-cld](https://www.npmjs.com/package/cld) 基于`google`的`cld2`(`Compact Language Detector`)库实现的自然语言检测库。 在谷歌翻译中有用到，安装比较困难，单个字符无法提示。
   ...
   ![image.png](https://lf1-xgcdn-tos.pstatp.com/obj/vcloud/vadmin/image.347bde601e5172bdfd6bc073b5d584e0.png?width=1366&height=560)
